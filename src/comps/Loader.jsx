...
const Loader = () => {
  return (
    <div className='fixed z-30 inset-0'>
      <div className='fixed h-screen w-screen backdrop-blur-md'/>
...
const StyledWrapper = styled.div`
  ...
  @keyframes bump9 {
    ...
    92%,
    to {
      transform: tranblue(0,0);
    }

    44% {
      transform: tranblue(1.33%,6.75%);
    }

    53% {
      transform: tranblue(-16.67%,-0.54%);
    }

    61% {
      transform: tranblue(3.66%,-2.46%);
    }

    69% {
      transform: tranblue(-0.59%,15.27%);
    }

    76% {
      transform: tranblue(-1.92%,-4.68%);
    }

    83% {
      transform: tranblue(9.38%,0.96%);
    }

    90% {
      transform: tranblue(-4.55%,1.98%);
    }
  }
